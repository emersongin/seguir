...
import AccountGateway, { ProcessPaymentInputDto, GetTransactionOutputDto } from './PaymentGateway';
...
export default class PaymentGatewayHttp implements PaymentGateway {
  async processPayment(input: ProcessPaymentInputDto): Promise<void> {
    const response = await axios.post('http://localhost:3002/payment_process', input);
    if (response.status === 422) throw new Error('Payment failed');
  }
  
  async getTransactionByRideId(rideId: string): Promise<GetTransactionOutputDto | undefined> {
